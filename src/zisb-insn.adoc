...
When the Zihpd extension is implemented, predictions learned in one hardware
...
software must execute `fence.time` to ensure that predictions created by code
that previously executed in that hardware protection domain do not affect
outcomes for code that subsequently executes in that hardware protection
domain.

[NOTE]
====
When Zihpd is implemented, the predictors for control-flow, data-dependency, and
...
Executing `fence.time` may incur substantial overhead. When Zihpd is implemented,
...
== "Zisb" - Speculation Barrier Instructions, Version 0.1
